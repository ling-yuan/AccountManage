...
mainWindow.geometry('800x500+%d+%d' % ((screenwidth - 800) / 2,
...
def number_of_records():
    ...
    num = db.executeQuery(sql_count_record)[0][0]  # [(0,)]
    return num


# 方法: 刷新记录数量
def refresh_records_num():
    global tmp_num
    global text_label_num
    tmp_num = number_of_records()
    text_label_num.set(f"共有 {tmp_num} 条记录")
    return


# 事件: 删除列表中所有元素
def del_info_in_table():
    global tree
    x = tree.get_children()
    for item in x:
        tree.delete(item)
...
# 事件: 刷新列表中项目
def add_info_in_table():
    global tree
    del_info_in_table()
    ans = db.executeQuery('select * from information')
    tmp_i = 1  # 设置编号
    for i in ans:
        tree.insert("",
                    "end",
...
def btn_inpwd_func():
    ...
    try:
        ...
        if t == 'xls' or t == 'xlsx':
            ...
            db.executeUpdate(sql_clear_table)
            # 放入导入的数据
            sql_add_info = 'insert into information values(?,?,?,?,?)'
            db.executeUpdate(sql_add_info, indb)
        elif t == 'txt':
            # 准备空列表放数据
            indb = []
            # 打开文件
        ...
        db.executeUpdate(sql_clear_table)
        sql_add_info = 'insert into information values(?,?,?,?,?)'
        db.executeUpdate(sql_add_info, ans)
...
def btn_del_func():
    ...
    for item in tree.selection():  #逐条删除
        item_text = tree.item(item, "values")
        db.executeUpdate(sql_delete_item,
...
# 事件: 搜索项目
def btn_searchinfo_func():
    # 获取搜索内容
    search = entry_search.get().lower()
    # 获取搜索类型
    select = combobox_selected.get()
    # 获取所有数据
    ans = db.executeQuery("select * from information")
    # 保存查找到的项目
    ans_list = []
    if select == '名称':
        ...
        for i in range(0, len(ans)):
            if search in ans[i][4].lower():
                ans_list.append(str(i + 1))
    tree.selection_set(ans_list)
    # entry_search.delete(0, tk.END)
    # tree.selection_add(iid)
    # tree.selection_set(["1", "8"])
    return
...
# 事件:单击表格中元素的单击事件
def tree_item_Click(event):
    global tree
    # tree.selection() 获取到所有选中
    ...
    entry_account.insert(0, item_text[2])
    entry_password.insert(0, item_text[3])
...
# Button:导入密码库
btn_inpwd = tk.Button(mainWindow,
                      text="导入密码库",
                      width=13,
                      height=1,
                      command=btn_inpwd_func)
btn_inpwd.place(x=50, y=190)

# Button:导出密码库
btn_outpwd = tk.Button(mainWindow,
                       text="导出密码库",
                       width=13,
                       height=1,
                       command=btn_outpwd_func)
...
# Button:清空输入信息框
btn_clear_entry = tk.Button(mainWindow,
                            text="清空",
                            width=8,
                            height=1,
                            command=btn_clear)
btn_clear_entry.place(x=710, y=75)

# Button:添加/修改项目
btn_add_info = tk.Button(mainWindow,
                         text="添加/修改",
                         width=8,
                         height=1,
                         command=btn_add_func)
btn_add_info.place(x=260, y=160)

# Button:删除项目
btn_del_info = tk.Button(mainWindow,
                         text="删除",
                         width=8,
                         height=1,
                         command=btn_del_func)
...
btn_search_info = tk.Button(mainWindow,
                            image=img_search,
                            command=btn_searchinfo_func)
...
# tree.column(col, width=125, anchor='w')  #设置col列的宽度,anchor表示内容位置
...
add_info_in_table()
# 绑定单击离开事件===========
tree.bind('<ButtonRelease-1>', tree_item_Click)
